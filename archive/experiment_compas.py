...
# %%
data = pd.read_csv("dataset/compas-scores-raw.csv")
data.head()

# %%
races = ["African-American", "Caucasian", "Hispanic"]
data = data[data["Ethnic_Code_Text"].isin(races)]
...
cover_columns = {
    ...
    "MaritalStatus": ["Single", "Married", "Divorced", "Separated", "Significant Other", "Widowed"],
    "LegalStatus": ["Pretrial", "Post Sentence", "Other", "Conditional Release", "Probation Violator"],
...
def make_sets(data, cover_columns, points):
    ...
    for index, row in data.iterrows():
        ...
        for col in cover_columns.keys():
            if row[col] in cover_columns[col]:
                this_set.append(points.index(row[col]))
        sets.append(set(this_set))
        colors.append(races.index(row["Ethnic_Code_Text"]))
    return sets, colors
...
verify_sets(sets, points)

# %%
np.unique(np.array(colors), return_counts=True), df["Ethnic_Code_Text"].value_counts()
...
runs_per_n = 5
N = len(points)
set_sizes = [200, 500, 750, 1000, 1200]
...
result_df_counts.to_csv(f"result_df_counts_compas.csv")
result_df_fairness.to_csv(f"result_df_fairness_compas.csv")
result_df_times.to_csv(f"result_df_times_compas.csv")